...
namespace App.compoment.Cache.Redis
{
    public class RedisRepository : IRedisRepository
    {
        ...
        public bool Set(string key, object value, DateTime expiry)
        {
            if (value is string cacheValue)
            {
                return  _database.StringSet(key, cacheValue, expiry - DateTime.Now);
            }
            else
            {
                return  _database.StringSet(key, value.SerializeObject(), expiry - DateTime.Now);
            }
        }
        ...
        public bool Set(string key, object value, TimeSpan? expiry = null)
        {
            if (value is string cacheValue)
            {
                return  _database.StringSet(key, cacheValue, expiry);
            }
            else
            {
                return  _database.StringSet(key, value.SerializeObject(), expiry);
            }
        }
        #endregion
